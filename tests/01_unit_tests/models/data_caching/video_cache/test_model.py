...
from censor_engine.models.data_caching.video_cache.model import VideoCache
from censor_engine.models.data_caching._abstract_model import Query
from censor_engine.models.data_caching._common_schemas import AIOutputData, CacheData
...
def make_data(frame=1, model="model_a"):
    return CacheData(cache_data=[AIOutputData(
        model_name=model,
        frame=frame,
        output_data=[],
    )])
...
def test_db_is_created(cache):
    db_file = cache._cache_path / "video_data.db"
    assert db_file.exists()


def test_save_and_get(cache):
...
def test_peak_returns_true_when_exists(cache):
...
def test_peak_returns_false_when_missing(cache):
    assert cache.peak(Query(model_name="missing", frame=999)) is False


def test_get_missing_raises(cache):
    with pytest.raises(ValueError) as exc:
        cache.get(Query(model_name="nope", frame=123))

    assert "Missing Frame Data" in str(exc.value)


def test_upsert_overwrites_existing_row(cache):
...
def test_multiple_models_same_frame(cache):
