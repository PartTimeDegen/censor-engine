...
from censor_engine.models.models_core.data_caching.image_cache.model import (
    ImageCache,
)
from censor_engine.models.models_core.data_caching.image_cache._paths import (
    IMAGE_OUTPUT,
)
...
DUMMY_DATA = AIOutputData(
    model_name="dummy",
    output_data=[
        DetectorOutput(origin="dummy", part_id=0, label="dummy_0", score=0.67),
        DetectorOutput(origin="dummy", part_id=1, label="dummy_1", score=0.67),
        DetectorOutput(origin="dummy", part_id=2, label="dummy_2", score=0.67),
        DetectorOutput(origin="dummy", part_id=3, label="dummy_3", score=0.67),
    ],
)
...
class TestImageCacheSave:
    def test_creates_output_file(self, image_cache):
        cache_data = CacheData(cache_data=[DUMMY_DATA])
    ...
    def test_writes_cache_data_to_file(self, image_cache):
        expected = CacheData(cache_data=[DUMMY_DATA])
