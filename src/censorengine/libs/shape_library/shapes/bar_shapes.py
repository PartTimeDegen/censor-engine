...
# TODO: In the shape handler, make joint_box used for each shape
...
class Bar(BarShape):
    ...
    def generate(
        ...
        LANDSCAPE_QUALIFIER = 1.2
        HEIGHT = dict_info["file_image"].shape[0]
        WIDTH = dict_info["file_image"].shape[1]
        is_image_landscape = WIDTH > (LANDSCAPE_QUALIFIER * HEIGHT)

        # # Calculate Cutoffs
        CUTOFF_HORIZONTAL_GRADIENT = 0.2
        is_gradient_in_horizontal_cutoff = (
            abs(gradient) < CUTOFF_HORIZONTAL_GRADIENT
        )
        ...
        if dict_info.get("bar_grad", None):
            gradient = dict_info["bar_grad"]
            dict_info["force_already_determined"] = True
        else:
            dict_info["bar_grad"] = gradient
            dict_info["force_already_determined"] = False
            dict_info["force_horizontal"] = False
            dict_info["force_vertical"] = False
        ...
        if not dict_info["force_already_determined"]:
            ...
            if force_horizontal or should_be_horizontal:
                dict_info["force_horizontal"] = True

            if force_vertical or should_be_vertical:
                dict_info["force_vertical"] = True

        if dict_info["force_horizontal"] and dict_info["force_vertical"]:
            if force_horizontal:
                dict_info["force_vertical"] = False
            elif force_vertical:
                dict_info["force_horizontal"] = False
        ...
        # # Vertical
        elif dict_info["force_vertical"]:
            # List Coords for Readability
            x_left = dict_corners["top_point"][0]
            x_right = dict_corners["bottom_point"][0]
            y_top = 0
            y_bottom = dict_info["file_image"].shape[0]
        ...
        # # Angled
            ...
            elif dict_corners["tilt"] == 0 and dict_info["bar_grad"] > 0:
            ...
            bar_points = [
                ...
                self._line(
                    gradient=dict_info["bar_grad"],
                    coords=top_left_bottom_right_point,
                    x_coord_to_find_y=dict_info["file_image"].shape[0],
                ),  # Bottom Right Corner
                self._line(
                    gradient=dict_info["bar_grad"],
                    coords=top_right_bottom_left_point,
